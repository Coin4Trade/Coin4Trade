...
*Setup instructions for a gitian build of C4T using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the C4T
...
Clone the git repositories for C4T and gitian and then checkout the C4T version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/Coin4Trade/C4T.git
cd C4T
...
Follow the instructions in [doc/release-process.md](release-process.md) in the C4T repository
...
To build C4T (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the C4T repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/C4T/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/Coin4Trade/C4T
...
For example:
```bash
URL=https://github.com/crowning-/C4T.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit C4T=${COMMIT} --url C4T=${URL} ../C4T/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit C4T=${COMMIT} --url C4T=${URL} ../C4T/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit C4T=${COMMIT} --url C4T=${URL} ../C4T/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/C4T-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/C4T-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/C4T-build.assert
```
...
[C4T/gitian.sigs](https://github.com/C4T-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
