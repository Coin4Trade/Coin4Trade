...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (tx.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CZC4TStake(spend));
    } else {
        ...
        CC4TStake* C4TInput = new CC4TStake();
        C4TInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(C4TInput);
    }
