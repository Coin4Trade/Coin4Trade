...
CzC4TTracker::CzC4TTracker(std::string strWalletFile)
{
...
CzC4TTracker::~CzC4TTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzC4TTracker::Init()
{
...
bool CzC4TTracker::Archive(CMintMeta& meta)
{
...
bool CzC4TTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzC4TTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzC4TTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzC4TTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzC4TTracker::GetSerialHashes()
{
...
CAmount CzC4TTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzC4TTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzC4TTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzC4TTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzC4TTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzC4TTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzC4TTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzC4TTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzC4TTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzC4TTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzC4TTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzC4TTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzC4TTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzC4TTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzC4TTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzC4TTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzC4TTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzC4T from DB\n", __func__, listDeterministicDB.size());
    }
