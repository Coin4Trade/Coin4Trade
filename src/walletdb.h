...
public:
    ...
    bool WriteZC4TSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZC4TSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZC4TSeed_deprecated(uint256& seed);
    bool EraseZC4TSeed();
    bool EraseZC4TSeed_deprecated();

    bool WriteZC4TCount(const uint32_t& nCount);
    bool ReadZC4TCount(uint32_t& nCount);
