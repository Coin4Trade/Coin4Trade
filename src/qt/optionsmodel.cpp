...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::C4T);
    ...
    if (!settings.contains("nAnonymizeC4TAmount"))
        settings.setValue("nAnonymizeC4TAmount", 1000);

    nAnonymizeC4TAmount = settings.value("nAnonymizeC4TAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeC4TAmount"))
        SoftSetArg("-anonymizeC4Tamount", settings.value("nAnonymizeC4TAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in C4T.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeC4TAmount:
            return QVariant(nAnonymizeC4TAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeC4TAmount:
            nAnonymizeC4TAmount = value.toInt();
            settings.setValue("nAnonymizeC4TAmount", nAnonymizeC4TAmount);
            emit anonymizeC4TAmountChanged(nAnonymizeC4TAmount);
            break;
        case CoinControlFeatures:
