...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 C4T.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal C4T:URI (C4T:XyZ...?message=example)
            {
...
static void NotifyzC4TReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzC4TReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzC4TReset.connect(boost::bind(NotifyzC4TReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzC4TReset.disconnect(boost::bind(NotifyzC4TReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zC4TTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
