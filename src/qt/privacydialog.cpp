                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zC4T ought to be enough for anybody." - Bill Gates, 2017
    ui->zC4TpayAmount->setValidator( new QDoubleValidator(0.0, 14190000.0, 20, this) );
    ...
    ui->labelzC4TSyncStatus->setText("(" + tr("out of sync") + ")");
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zC4TpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzC4T_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zC4T is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Mint_zC4T, true));
        if (!ctx.isValid()) {
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zC4T...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zC4T in ") +
...
void PrivacyDialog::on_pushButtonSpendzC4T_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zC4T is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Send_zC4T, true));
        if (!ctx.isValid()) {
            // Unlock wallet was cancelled
            return;
        }
        // Wallet is unlocked now, sedn zC4T
        sendzC4T();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zC4T
    sendzC4T();
}

void PrivacyDialog::on_pushButtonZC4TControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;

    ZC4TControlDialog* zC4TControl = new ZC4TControlDialog(this);
    zC4TControl->setModel(walletModel);
    zC4TControl->exec();
}

void PrivacyDialog::setZC4TControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzC4TSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzC4T()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid C4T Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zC4TpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zC4T is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " C4T </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zC4T</b>";
    ...
    // use mints from zC4T selector if applicable
    vector<CMintMeta> vMintsToFetch;
    vector<CZerocoinMint> vMintsSelected;
    if (!ZC4TControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZC4TControlDialog::GetSelectedMints();
        ...
        for (auto& meta : vMintsToFetch) {
            if (meta.nVersion < libzerocoin::PrivateCoin::PUBKEY_VERSION) {
                ...
                if (nSecurityLevel < 100) {
                    QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zC4T require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
                    ui->TEMintStatus->setPlainText(tr("Failed to spend zC4T"));
                    ui->TEMintStatus->repaint();
                    return;
                }
            }
    ...
    if (!fSuccess) {
        if (receipt.GetStatus() == ZC4T_SPEND_V1_SEC_LEVEL) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zC4T require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
            ui->TEMintStatus->setPlainText(tr("Failed to spend zC4T"));
            ui->TEMintStatus->repaint();
            return;
        ...
        const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zC4T transaction
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zC4T was spent successfully update the addressbook with the label
    ...
    // Clear zC4T selector in case it was used
    ZC4TControlDialog::setSelectedMints.clear();
    ui->labelzC4TSelected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zC4T Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " C4T, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zC4T Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    std::vector<CMintMeta> vMints = pwalletMain->zC4TTracker->GetMints(false);
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zC4T </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zC4T "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zC4T "));
    ui->labelzC4TAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
    ...
    ui->labelZsupplyAmount->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zC4T </b> "));
    for (auto denom : libzerocoin::zerocoinDenomList) {
        ...
        QString strSupply = QString::number(nSupply) + " x " + QString::number(denom) + " = <b>" +
                            QString::number(nSupply*denom) + " zC4T </b> ";
        switch (denom) {
            case libzerocoin::CoinDenomination::ZQ_ONE:
...
{
    ui->labelzC4TSyncStatus->setVisible(fShow);
}
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    bool fButtonsEnabled =  ui->pushButtonMintzC4T->isEnabled();
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        // Mint zC4T
        ui->pushButtonMintzC4T->setEnabled(false);
        ui->pushButtonMintzC4T->setToolTip(tr("zC4T is currently disabled due to maintenance."));

        // Spend zC4T
        ui->pushButtonSpendzC4T->setEnabled(false);
        ui->pushButtonSpendzC4T->setToolTip(tr("zC4T is currently disabled due to maintenance."));
    } else if (!fMaintenanceMode && !fButtonsEnabled) {
        // Mint zC4T
        ui->pushButtonMintzC4T->setEnabled(true);
        ui->pushButtonMintzC4T->setToolTip(tr("PrivacyDialog", "Enter an amount of C4T to convert to zC4T", 0));

        // Spend zC4T
        ui->pushButtonSpendzC4T->setEnabled(true);
        ui->pushButtonSpendzC4T->setToolTip(tr("Spend Zerocoin. Without 'Pay To:' address creates payments to yourself."));
    }
}
