...
// Copyright (c) 2017-2018 The C4T developers
// Distributed under the MIT/X11 software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include "zC4Tcontroldialog.h"
#include "ui_zC4Tcontroldialog.h"
...
std::set<std::string> ZC4TControlDialog::setSelectedMints;
std::set<CMintMeta> ZC4TControlDialog::setMints;

ZC4TControlDialog::ZC4TControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZC4TControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZC4TControlDialog::~ZC4TControlDialog()
{
    delete ui;
}

void ZC4TControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZC4TControlDialog::updateList()
{
...
void ZC4TControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZC4TControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZC4T_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

}

std::vector<CMintMeta> ZC4TControlDialog::GetSelectedMints()
{
...
void ZC4TControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
