...
CzC4TWallet::CzC4TWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zC4T seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZC4TSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZC4TSeed_deprecated()) {
                    LogPrintf("%s: Updated zC4Tseed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zC4T seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zC4T wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzC4TWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZC4TCount(nCountLastUsed);
    else if (!walletdb.ReadZC4TCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzC4TWallet::Lock()
{
    seedMaster = 0;
}

void CzC4TWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzC4TWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZC4T(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzC4TWallet::LoadMintPoolFromDB()
{
...
void CzC4TWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzC4TWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzC4TWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zC4TTracker->HasPubcoinHash(pMint.first)) {
...
bool CzC4TWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZC4T(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zC4TTracker which also adds to database
    pwalletMain->zC4TTracker->Add(dMint, true);
...
void CzC4TWallet::SeedToZC4T(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzC4TWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzC4TWallet::GenerateDeterministicZC4T(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzC4TWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzC4TWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
