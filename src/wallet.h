...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT1000IFMN = 3,
    ONLY_NONDENOMINATED_NOT1000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 C4Tat the same time
    ONLY_1000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zC4Tsend
enum ZerocoinSpendStatus {
    ZC4T_SPEND_OKAY = 0,                            // No error
    ZC4T_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZC4T_WALLET_LOCKED = 2,                         // Wallet was locked
    ZC4T_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZC4T_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZC4T_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZC4T_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZC4T_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZC4T_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZC4T_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZC4T_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZC4T_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZC4T_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZC4T_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZC4T_SPENT_USED_ZC4T= 14,                      // Coin has already been spend
    ZC4T_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZC4T_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZC4TBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZC4TOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzC4TAuto) const;
    ...
    */
    ...
    CzC4TWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzC4TTracker> zC4TTracker;
    ...
    void setZWallet(CzC4TWallet* zwallet)
    {
        zwalletMain = zwallet;
        zC4TTracker = std::unique_ptr<CzC4TTracker>(new CzC4TTracker(strWalletFile));
    }

    CzC4TWallet* getZWallet() { return zwalletMain; }
    ...
    void setZC4TAutoBackups(bool fEnabled)
    {
    ...
    /** zC4Treset */
    boost::signals2::signal<void()> NotifyzC4TReset;
